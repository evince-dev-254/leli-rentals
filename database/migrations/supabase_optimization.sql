-- ==============================================================================
-- SUPABASE OPTIMIZATION MIGRATION (Phase 7 - SURGICAL LINTER SATISFACTION)
-- Resolves the final Linter Warnings ("Always True" RLS & extension schema).
-- ==============================================================================
...
-- Moving pg_net to extensions schema to clear the dashboard warning.
...
-- 5. SURGICAL LINTER SATISFACTION (Non-Broad USING clauses)
-- Replacing USING (true) with specific filters to satisfy the "Always True" linter.
...
USING (account_status = 'active' OR (SELECT public.is_admin())) 
WITH CHECK ((SELECT public.is_admin()) OR (SELECT auth.uid()) = id);

-- [Listings]
CREATE POLICY "listings_anon_read" ON public.listings FOR SELECT TO anon 
...
USING ((SELECT auth.uid()) IN (renter_id, owner_id) OR (SELECT public.is_admin()))
WITH CHECK ((SELECT auth.uid()) = renter_id OR (SELECT public.is_admin()));
...
CREATE POLICY "subcategories_anon_read" ON public.subcategories FOR SELECT TO anon 
USING (is_active = true);
...
USING ((SELECT auth.uid()) IN (participant_1_id, participant_2_id));
...
USING (affiliate_id = (SELECT auth.uid()) OR (SELECT public.is_admin()));
...
USING (status = 'published' OR reviewer_id = (SELECT auth.uid()) OR (SELECT public.is_admin()))
