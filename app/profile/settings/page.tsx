...
import {
  ...
  Smartphone, Mail as MailIcon, MessageSquare, Edit, Award, Star
} from "lucide-react"
...
import GovernmentIdUpload from "@/components/government-id-upload"
...
export default function AccountSettingsPage() {
  ...
  useEffect(() => {
    const loadUserSettings = async () => {
      ...
      // Load account type
      const currentAccountType = getUserAccountType()
  ...
  const handleProfileUpdate = async (e: React.FormEvent) => {
    ...
    try {
      await userSettingsService.updateProfile(user.uid, profileData)
  ...
  const handleImageChange = async (file: File): Promise<boolean> => {
    ...
    try {
      const result = await userProfileService.updateProfileImage(user.uid, file)
      if (result.success && result.url) {
        // Update local state with new avatar URL
        setProfileData(prev => ({ ...prev, avatar: result.url || "" }))

        // Also update userSettings to keep it in sync
        await userSettingsService.updateProfile(user.uid, { avatar: result.url })
        // Update local userSettings state
        if (userSettings) {
          setUserSettings({
            ...userSettings,
            profile: { ...userSettings.profile, avatar: result.url }
          })
        }
  ...
  const handleImageRemove = async (): Promise<boolean> => {
    ...
    try {
      const result = await userProfileService.deleteProfileImage(user.uid, profileData.avatar)
      if (result.success) {
  ...
  const handleNotificationToggle = async (setting: string, value: boolean) => {
    ...
    try {
      await userSettingsService.updateNotificationSettings(user.uid, { [setting]: value })
  ...
  const handlePrivacyToggle = async (setting: string, value: any) => {
    ...
    try {
      await userSettingsService.updatePrivacySettings(user.uid, { [setting]: value })
  ...
  const handleAccountTypeChange = (newAccountType: string) => {
    setUserAccountType(newAccountType as 'renter' | 'owner')
    setAccountType(newAccountType)

    toast({
      title: "Account type updated",
      description: `Your account is now set as ${newAccountType === 'renter' ? 'Renter' : 'Owner'}.`,
    })
  }
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card className="border-0 shadow-lg">
              ...
              <CardContent className="space-y-6">
                <ImageUpload
                  ...
                  userName={profileData.name || user.displayName || "User"}
            ...
            <Card className="border-0 shadow-lg">
              ...
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  <Label className="text-sm font-medium">Current Account Type</Label>
                  <div className="flex items-center gap-4">
                    <div className="flex items-center space-x-2">
                      <input
                        ...
                        name="accountType"
                        value="renter"
                        checked={accountType === 'renter'}
                        onChange={(e) => handleAccountTypeChange(e.target.value)}
                        className="text-blue-600"
                        aria-label="Switch to renter account"
                      />
                      <Label htmlFor="renter" className="text-sm">Renter - I want to rent items</Label>
                    </div>
                    <div className="flex items-center space-x-2">
                      <input
                        type="radio"
                        ...
                        aria-label="Switch to owner account"
                      />
                      <Label htmlFor="owner" className="text-sm">Owner - I want to list items</Label>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-4 bg-blue-50 rounded-lg">
                  <p className="text-sm text-blue-800">
                    <strong>Note:</strong> Changing your account type will update your dashboard and available features.
                    {accountType === 'renter' ? ' As a renter, you can browse and book listings.' : ' As an owner, you can create and manage your listings.'}
                  </p>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="border-0 shadow-lg">
              <CardHeader>
                ...
                <CardDescription>Complete your verification to unlock premium features and priority booking access</CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                {/* Verification Status */}
                <div className={`p-4 rounded-lg ${
                  userSettings?.verification?.isVerified
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className={`font-medium ${
                        userSettings?.verification?.isVerified ? 'text-green-800' : 'text-blue-800'
                      }`}>
                        {userSettings?.verification?.isVerified ? 'Verified Account' : 'Unverified Account'}
                      </div>
                      <div className={`text-sm ${
                        userSettings?.verification?.isVerified ? 'text-green-600' : 'text-blue-600'
                      }`}>
                        {userSettings?.verification?.isVerified
                          ? 'You have access to priority bookings and premium features'
                          : 'Complete verification to get priority in bookings and access premium features'
                        }
                      </div>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div className={`p-4 rounded-lg border ${
                    userSettings?.verification?.emailVerified
                    ...
                    <div className="text-xs text-muted-foreground">
                      {userSettings?.verification?.emailVerified ? 'Verified' : 'Pending'}
                    </div>
                  </div>

                  {/* Phone Verification */}
                  <div className={`p-4 rounded-lg border ${
                    userSettings?.verification?.phoneVerified
                      ? 'bg-green-50 border-green-200'
                      : 'bg-gray-50 border-gray-200'
                  }`}>
                    <div className="flex items-center gap-2 mb-2">
                    ...
                    <div className="text-xs text-muted-foreground">
                      {userSettings?.verification?.phoneVerified ? 'Verified' : 'Pending'}
                    </div>
                  </div>

                  {/* ID Verification */}
                  <div className={`p-4 rounded-lg border ${
                    userSettings?.verification?.idVerified
                      ? 'bg-green-50 border-green-200'
                      : 'bg-gray-50 border-gray-200'
                  }`}>
                    <div className="flex items-center gap-2 mb-2">
                    ...
                    <div className="text-xs text-muted-foreground">
                      {userSettings?.verification?.idVerified ? 'Verified' : 'Pending'}
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <PhoneVerification
                    ...
                    isVerified={userSettings?.verification?.phoneVerified || false}
                    onVerificationChange={(verified) => {
                      if (userSettings) {
                        setUserSettings({
                          ...userSettings,
                          verification: { ...userSettings.verification, phoneVerified: verified }
                        })
                      }
                    }}
                  />
                  ...
                  <GovernmentIdUpload
                    userId={user?.uid || ""}
                    currentIdUrl={userSettings?.verification?.governmentIdUrl}
                    isVerified={userSettings?.verification?.idVerified || false}
                    onVerificationChange={(verified) => {
                      if (userSettings) {
                        setUserSettings({
                          ...userSettings,
                          verification: { ...userSettings.verification, idVerified: verified }
                        })
                      }
                    }}
                  />
